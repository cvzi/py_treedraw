...
class Tree:
  ...
  def _executeShifts(self,v):
    shift = 0
    change = 0
    i = len(v.children)
    for i in xrange(len(v.children)-1,-1,-1):
      w = v.children[i]
  ...
  def _ancestor(self,v_i,v,defaultAncestor):
  ...
  def removeChild(self,v):
    ...
    for i in xrange(len(self.children)):
      if self.children[i] == v:
        del self.children[i]
        j = 1
    for i in xrange(len(self.tree.nodes)):
    ...
    # Update numbers
    for i in xrange(j,len(self.children)):
      self.children[i] = i
...
if __name__ == '__main__': 
  ...
  for node in T.nodes:
    if node.parent:
      pygame.draw.aaline(screen, (0, 0, 0), node.position(rootpos,sizex,sizey), node.parent.position(rootpos,sizex,sizey))
  ...
  for node in T.nodes:
    ...
    pygame.gfxdraw.aaellipse(screen, p.centerx, p.centery, int(p.width/2), int(p.height/2), (0,255,255))
